...
```
Mac Mini M4 (192.168.1.188)        ← OpenClaw 大脑 (控制面)
    │
    │  HTTP (HMAC 签名)
    ▼
192.168.1.139:8001 (FastAPI)       ← Bridge API + 回测沙箱 (执行面)
    │
    │  内部
    ▼
PostgreSQL / Redis                  ← 数据资产
```

| 层 | IP | 角色 |
|----|-----|------|
| 控制面 | 192.168.1.188 (Mac Mini) | AI Agent、策略研究、风控决策 |
| 执行面 | 192.168.1.139 | 数据 API、沙箱回测、策略归档 |
| 展示面 | 192.168.1.138 | Next.js 前端、Nginx 反向代理 |
...
**禁止的模式**：`__import__()`, `eval()`, `exec()`, `compile()`, `globals()`, `locals()`, `getattr()`, `setattr()`, `delattr()`, `open(..., 'w')`, `open(..., 'a')`

**禁止的外部数据库**：`yfinance`, `akshare`, `tushare`
...
| `run_backtest(code, stock?, start_date?, end_date?, timeout?)` | /backtest/run/ | Backtest Agent |
...
| POST | `/api/bridge/backtest/run/` | 沙箱回测执行 |
...
| **Backtest Engine** | `/bridge/backtest/run/` | 提交代码到沙箱执行 |
