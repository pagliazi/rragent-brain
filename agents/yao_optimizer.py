...
Redis key 设计:
  rragent:yao:theme_stats      Hash  主题性能统计
  rragent:yao:iteration_log    List  迭代事件日志 (最近 100 条)
  rragent:yao:signals:latest   String  最新股票信号 JSON
  rragent:yao:dashboard_cache  String  dashboard 缓存 (TTL=300s)
"""
...
logger = logging.getLogger("agent.yao_optimizer")

REDIS_KEY_THEME_STATS    = "rragent:yao:theme_stats"
REDIS_KEY_ITER_LOG       = "rragent:yao:iteration_log"
REDIS_KEY_SIGNALS        = "rragent:yao:signals:latest"
REDIS_KEY_DASH_CACHE     = "rragent:yao:dashboard_cache"
REDIS_KEY_THEME_WEIGHTS  = "rragent:yao:theme_weights"

# 主题中文名映射
THEME_NAMES = {
    "yao_amplitude_compression": "振幅收窄蓄势",
    "yao_volume_dry_up":         "缩量蓄势",
    "yao_limit_up_momentum":     "涨停板动量",
    "yao_breakout_signal":       "放量突破",
    "yao_opening_attack":        "竞价强势",
    "yao_chip_consolidation":    "筹码整理",
    "yao_vol_price_resonance":   "量价共振",
    "yao_intraday_high_close":   "尾盘强势",
    "yao_pre_explosion_pattern": "爆发前夕",
    "yao_gap_follow":            "跳空延续",
    "yao_mutation":              "变异进化",
}
...
async def analyze_library(factor_lib, r=None) -> dict:
    ...
    yao_factors = [f for f in all_factors if f.theme.startswith("yao_")]

    # ── 主题统计 ──
    theme_map: dict[str, list] = {}
    for f in yao_factors:
        tid = f.theme
    ...
    # ── 最优因子 TOP 10 ──
    top_factors = sorted(yao_factors, key=lambda f: f.sharpe, reverse=True)[:10]
    top_list = []
    ...
    # ── 全局摘要 ──
    all_sharpes   = [f.sharpe   for f in yao_factors if f.sharpe   > 0]
    all_win_rates = [f.win_rate for f in yao_factors if f.win_rate > 0]
    summary = {
        "total_factors":   len(yao_factors),
...
async def run_analysis_and_update(factor_lib, r, notify_fn=None) -> dict:
    ...
    # 从未出现的主题保留基础权重
    from agents.yao_digger import YAO_THEMES
    for t in YAO_THEMES:
...
async def refresh_signals(factor_lib, bridge, r, top_n: int = 3, notify_fn=None) -> list:
    ...
    top_factors = sorted(yao_factors, key=lambda f: f.ir, reverse=True)[:top_n]
...
async def get_theme_weights(r) -> dict:
    ...
    # 默认: 所有主题等权
    from agents.yao_digger import YAO_THEMES
    return {t["id"]: 1.0 for t in YAO_THEMES}
