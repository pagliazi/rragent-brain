"""
妖股因子挖掘器 — Yao Stock Factor Digger
...
架构位置:
  yao_digger.run_yao_session() → 同 alpha_digger 的底层 bridge/router/factor_lib 基础设施
...
logger = logging.getLogger("agent.yao_digger")
...
█ 背景知识: 什么是妖股
  ...
  - 连续涨停板 (close >= prev_close × 1.099，即涨停价判定)
  - 启动前有明显蓄势期: 振幅收窄、量能萎缩、筹码集中
  - 启动时放量突破: 成交量是均量的 3-5 倍
  - 小盘特征: 换手率高，情绪驱动为主

█ 你的任务: 挖掘「启动前 1-3 天」的预测信号
  不是追涨，不是识别已经涨停的股票，而是找到「即将启动」的特征。
...
█ 关键量价信号参考 (直接用于构建因子):
  涨停判定    : limit_up  = (close >= close.shift(1) * 1.099).astype(float)
  跌停判定    : limit_dn  = (close <= close.shift(1) * 0.901).astype(float)
  振幅        : amp       = (high - low) / (close.shift(1) + 1e-8)
  量比 (近似) : vol_ratio = volume / (volume.rolling(20).mean() + 1e-8)
  竞价跳空    : gap       = (open - close.shift(1)) / (close.shift(1) + 1e-8)
  日内强度    : body      = (close - open) / (high - low + 1e-8)
  收盘位置    : tail_pos  = (close - low) / (high - low + 1e-8)
  低点抬高    : low_trend = low.rolling(5).min().diff(3)

█ 随机参数种子: lookback={lookback}, weight_decay={decay_weight:.2f}

█ 数据接口:
  generate_factor(matrices) 中 matrices = {{
    'open': DataFrame, 'high': DataFrame, 'low': DataFrame,
    'close': DataFrame, 'volume': DataFrame
  }}  (index=trade_date, columns=ts_code)

█ 返回值:
  return factor  # DataFrame, shape 同 close
  因子值越高 = 该股票越可能是即将启动的妖股 (做多信号)

█ 可用库: numpy (np), pandas (pd) 仅此两个。

█ 因子设计硬性要求:
  1. 必须同时用到「价格维度」和「成交量维度」(纯价格或纯量无效)
  2. 至少组合 3 个不同的信号维度 (如 振幅+量比+收盘强度)
  3. 捕捉「蓄势→爆发」的转折点，而非连续趋势
  4. 使用至少 2 个不同的时序算子 (rolling/shift/diff/ewm)
  5. 所有中间变量 .fillna(0) 处理
  6. 避免使用传统指标名 (MACD/KDJ/RSI)，用原始数学公式

█ 妖股特有约束:
  - 信号窗口短: 主要 lookback ≤ 20 日 (妖股行情快进快出)
  - 高覆盖度: 因子在全市场有值，从中排序选出高分位候选
  - 不要过度追求对单一历史妖股的完美拟合 (过拟合=PBO>0.5，会被剔除)

{quality_rules}

只输出 generate_factor 函数完整代码:
...
def _generate_yao_seed(focus_theme_id: str | None = None) -> dict:
    """生成妖股挖掘种子 (随机主题 + 参数扰动)。
    ...
    if focus_theme_id:
        candidates = [t for t in YAO_THEMES if t["id"] == focus_theme_id]
        theme = candidates[0] if candidates else random.choice(YAO_THEMES)
    else:
        theme = random.choice(YAO_THEMES)
    ...
    prompt = YAO_MINER_PROMPT.format(
...
def _generate_yao_mutation_seed(original_code: str, metrics: dict) -> dict:
    """基于现有妖股因子做变异。"""
    from agents.factor_quality import QUALITY_RULES_FOR_PROMPT
...
async def run_yao_mining_round(
    router,
    bridge,
    factor_lib,
    notify_fn=None,
    ...
    from agents.alpha_digger import _extract_factor_code, _validate_factor_code

    # 生成种子: N-1 个随机妖股主题 + 1 个变异 (如果库中有妖股因子)
    seeds = [_generate_yao_seed(focus_theme_id) for _ in range(n_factors)]

    # 查找库中已有的最优妖股因子做变异
    existing = await factor_lib.get_all_factors()
    yao_factors = [f for f in existing if f.theme.startswith("yao_")]
    if yao_factors:
        best_yao = max(yao_factors, key=lambda f: f.ir)
        seeds[-1] = _generate_yao_mutation_seed(best_yao.code, {
            "sharpe": best_yao.sharpe, "mean_ic": best_yao.ic_mean, "ir": best_yao.ir,
        })
    ...
    for i, seed in enumerate(seeds):
        try:
            ...
            logger.warning("LLM 调用失败 (yao_seed %d): %s", i, e)
            continue
    ...
    for item in generated_codes:
        ...
        try:
            ...
            if pbo_score > 0.5:
                reason_text = (
                    f"[PB01] PBO={pbo_score:.3f} > 0.5 — 妖股因子跨时段不一致，过拟合拒绝"
...
async def run_yao_session(
    ...
    max_rounds = max_rounds or YAO_SESSION_CONFIG["max_rounds_per_session"]
    factors_per_round = factors_per_round or YAO_SESSION_CONFIG["factors_per_round"]
    interval = round_interval or YAO_SESSION_CONFIG["round_interval_sec"]
    ...
    theme_names = [t["name"] for t in YAO_THEMES]
    await _notify(
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        try:
            round_result = await run_yao_mining_round(
                router=router,
                bridge=bridge,
