...
function SystemView() {
  ...
  const toast = useContext(ToastContext);
  const refresh = async (cmd) => {
    try {
      setLoading(prev=>({...prev,[cmd]:true}));
      const r = await apiPost('/api/command', {cmd});
      setResults(prev=>({...prev,[cmd]:r && r.result!=null ? String(r.result) : (r && r.error ? 'Error' : '')}));
    } catch(e) {
      console.error('SystemView refresh error:', cmd, e);
      setResults(prev=>({...prev,[cmd]:'加载失败: '+e.message}));
    } finally {
      setLoading(prev=>({...prev,[cmd]:false}));
    }
  };
  const refreshAll = () => { panels.forEach(p => refresh(p.cmd)); };
  const runCustom = async () => { if (!customCmd.trim()) return; setCustomLoading(true); try { const r = await apiPost('/api/command', {cmd:customCmd.trim(),args:customArgs.trim()}); setCustomResult(r && r.result!=null ? String(r.result) : ''); toast('命令已执行', r && r.error ? 'error' : 'success'); } catch(e) { setCustomResult('Error: '+e.message); toast('执行失败','error'); } finally { setCustomLoading(false); } };
  useEffect(() => { try { refreshAll(); } catch(e) { console.error('SystemView init error:', e); setInitError(e.message); } }, []);
  if (initError) return <div className="flex-1 flex items-center justify-center text-red-400 p-8">SystemView 初始化失败: {initError}</div>;
  return (
